...
def _hip_gguf_cflags() -> list[str]:
    ...
    # Default to the established one-row configuration.  The two-row and
    # eight-wave configurations are opt-in candidate builds only.  Eight waves
    # mirrors the current llama.cpp RDNA4 policy for simple one-vector formats,
    # but it must independently pass FreeToken's model-quality and API gates
    # before it can affect any serving configuration.
    mmv_y = os.environ.get(_HIP_GGUF_MMV_Y_ENV, "1").strip()
    if mmv_y not in {"1", "2", "8"}:
        raise RuntimeError(
            f"{_HIP_GGUF_MMV_Y_ENV} must be 1, 2, or 8, got {mmv_y!r}"
        )
