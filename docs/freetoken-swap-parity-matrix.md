...
| Start, stop, switch, PID identity, re-adoption | Native manager is the sole process owner. Routed transitions, daemon shutdown, and legacy manual engine controls use the same coordinator; manual claims fail while routing owns or admits work. | Deterministic tests prove exact re-adoption, matching-token release, routed-lease conflict rejection, stop preemption with stale-token protection, routed admission waiting behind a blocked or client-disconnected manual start, failed-readiness rollback completing after client cancellation, and shutdown rejecting queued/new admission while draining active leases. Linux/current-engine evidence remains required. |
