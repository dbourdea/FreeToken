...
- Local deterministic verification on the current Windows checkout: 196 daemon
...
| Model catalog and lifecycle controls | Validated TOML catalog, collision-safe alternate IDs, unlisted profiles, authenticated profile endpoints, native process manager, and exact explicit/dynamic/omitted-default-port re-adoption | Implemented and CPU/HTTP tested |
