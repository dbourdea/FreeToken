...
| Streaming cold-load feedback | Pinned global/per-model `sendLoadingState` behavior has been source-inspected: streaming chat only, after concurrency admission, with queue/load reasoning SSE and in-band terminal errors | Applicable implementation is still missing; deterministic and native evidence required |
