...
- Local deterministic verification on the current Windows checkout: 153 daemon
...
| Client cancellation | Native opaque router request IDs, atomic duplicate-ID rejection before admission/upstream work, disconnect-aware queued admission, active-request list, explicit cancel endpoint, upstream socket close, lease release, and cancellation metrics. Failed, disconnected, or cancelled admission and failed upstream connection release ownership safely. | Deterministic HTTP tested; current native same-instance GPU verification required |
