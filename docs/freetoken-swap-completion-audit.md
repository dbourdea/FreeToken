...
- Local deterministic verification on the current Windows checkout: 258 daemon
...
- GitHub-hosted Ubuntu verification at
  `89d026c7253b93e512fd2792233a92133c79fb7f` (Actions run `34918087894`)
  reported 253 passed with zero failures, errors, or skips. This includes the
  ...
  listing coverage. It also executes the
...
| Model catalog and lifecycle controls | Validated TOML catalog, collision-safe slash-namespaced and colon-variant alternate IDs, ordered static JSON strip/hard/soft/by-ID filters with protected model routing, unlisted profiles, authenticated profile endpoints, native process manager, longest-prefix direct-upstream resolution, and exact explicit/dynamic/omitted-default-port re-adoption | Implemented and CPU/HTTP tested |
