...
def test_hip_gguf_flags_keep_the_one_row_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert flags == [
        "-O3",
        "-DGGML_CUDA_MMV_Y=1",
        "-DGGML_CUDA_Q8_MMV_WARPS=1",
        "-DGGML_CUDA_MOE_K_WARPS=8",
    ]
...
def test_hip_gguf_flags_reject_an_unreviewed_q8_wave_count(monkeypatch: pytest.MonkeyPatch) -> None:
    """The dense-Q8 candidate must not compile an arbitrary unqualified wave count."""

    monkeypatch.setenv("FREETOKEN_GGUF_Q8_MMV_WARPS", "4")

    with pytest.raises(RuntimeError, match="FREETOKEN_GGUF_Q8_MMV_WARPS must be 1 or 8"):
        gguf._hip_gguf_cflags()


def test_hip_gguf_flags_allow_only_the_reviewed_moe_k_wave_candidates(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Expose the four-wave routed-expert experiment and reject arbitrary launches."""

    monkeypatch.setenv("FREETOKEN_GGUF_MOE_K_WARPS", "4")
    monkeypatch.setenv("PYTORCH_ROCM_ARCH", "gfx1151")

    assert gguf._hip_gguf_cflags()[-1] == "-DGGML_CUDA_MOE_K_WARPS=4"

    monkeypatch.setenv("FREETOKEN_GGUF_MOE_K_WARPS", "6")

    with pytest.raises(RuntimeError, match="FREETOKEN_GGUF_MOE_K_WARPS must be 4 or 8"):
        gguf._hip_gguf_cflags()
