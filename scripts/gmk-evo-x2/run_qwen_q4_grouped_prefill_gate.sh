...
if [[ "${COMPONENT_CANDIDATE}" == "grouped" ]]; then
    ...
    if [[ "${COMPONENT_CANDIDATE}" == "three_rows" ]]; then
        ...
        row_name="three-rows"
    elif [[ "${COMPONENT_CANDIDATE}" == "four_rows" ]]; then
        # Preserve the same strict real-weight reference and compiler knobs as
        # lower-row trials. Only the candidate's output-row grouping changes.
        ...
        row_flag=(--vector-five-rows)
        row_env="FREETOKEN_GGUF_MOE_K_FIVE_ROWS=1"
        row_name="five-rows"
    fi
