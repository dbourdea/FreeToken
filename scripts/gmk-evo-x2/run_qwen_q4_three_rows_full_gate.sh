...
readonly MOE_K_FIVE_ROWS="${FREETOKEN_Q4_MOE_K_FIVE_ROWS:-0}"
...
[[ "${MOE_K_FIVE_ROWS}" == "0" || "${MOE_K_FIVE_ROWS}" == "1" ]] || { echo "FREETOKEN_Q4_MOE_K_FIVE_ROWS must be 0 or 1" >&2; exit 2; }
...
[[ $((MOE_K_TWO_ROWS + MOE_K_THREE_ROWS + MOE_K_FOUR_ROWS + MOE_K_FIVE_ROWS)) -le 1 ]] || { echo "select at most one Q4 row-sharing candidate" >&2; exit 2; }
...
FREETOKEN_GGUF_MOE_K_FIVE_ROWS="${MOE_K_FIVE_ROWS}" \
...
printf 'q4_candidate_quality_scheduler_and_c4=passed moe_k_two_rows=%s moe_k_three_rows=%s moe_k_four_rows=%s moe_k_five_rows=%s q4_k_four_rows=%s q5_k_four_rows=%s grouped_prefill_min_tokens=%s grouped_prefill_mode=%s\n' \
    "${MOE_K_TWO_ROWS}" "${MOE_K_THREE_ROWS}" "${MOE_K_FOUR_ROWS}" "${MOE_K_FIVE_ROWS}" "${Q4_K_FOUR_ROWS}" "${Q5_K_FOUR_ROWS}" "${GROUPED_PREFILL_MIN_TOKENS}" "${GROUPED_PREFILL_MODE}" >"${ARTIFACT_DIR}/result.txt"
